...
const formSubmitError =
  document.querySelector<HTMLParagraphElement>(".form-submit-error");
...
if (signInForm) {
  const signInBtn = signInForm.querySelector<HTMLButtonElement>("button");
  ...
  if (signInBtn) {
    ...
    const checkSignInForm = () => {
      ...
      const passwordValid =
        passwordIput.value.trim().length >= 6 &&
...
const rePassInput = document.getElementById(
  "repassword",
...
if (registerForm) {
  const registerBtn = registerForm.querySelector<HTMLButtonElement>("button");
  ...
  if (registerBtn) {
    ...
    const checkRegisterForm = () => {
      ...
      const rePasswordValid =
        rePassInput?.value.trim() === passwordIput.value.trim() &&
